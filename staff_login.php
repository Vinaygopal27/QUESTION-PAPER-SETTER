...
if (isset($_POST['btn'])) {
    if (isset($uname) && isset($pass)) {
        $qry = mysqli_query($connect, "SELECT * FROM staff_detail WHERE uname='$uname' AND pass='$pass'");
        $num = mysqli_num_rows($qry);

        if ($num == 1) {
            if ($row1 = mysqli_fetch_array($qry)) {
                $_SESSION['uid'] = $row1['id'];
            }
            $_SESSION['user'] = $uname;
            // Additional logic or actions can be added here
            echo '<script language="javascript">alert("Login Successfully"); window.location.href="staff_home.php";</script>';
        } else {
            echo '<script language="javascript">alert("Failed");</script>';
        }
    } else {
        echo '<script language="javascript">alert("Username and password are required");</script>';
    }
}
?>
			...
			<nav class="main_nav_container">
				<div class="main_nav">
					<ul class="main_nav_list">
						...
						<li class="main_nav_item"><a href="staff_login.php">staff</a></li>
					</ul>
				</div>
			</nav>
		</div>
					...
					<div class="section_title text-center">
						...
						<form name="form1" method="post" action="">
						  <table width="100%" border="0" align="center">
                            <tr>
                              <td width="31%">&nbsp;</td>
                              <td width="69%"><table width="352" height="178" border="0" align="center">
                                <tr>
                                  <td>Username</td>
                                  <td><label>
